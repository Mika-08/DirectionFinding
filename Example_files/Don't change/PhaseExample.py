...
def makePeak(ref, samples1):
    ...
    plt.plot(td, np.abs(ref), label="Reference signal")
    plt.plot(td, np.abs(samples1), label="Test signal")
    plt.title(F"Phase difference at 0.05 seconds time delay\n"
              F"Calculated value {phase_delay_degrees:.01f} degrees with added noise")
...
delay1, phase_delay = makePeak(modulated_ref, shifted)
phase_delay_degrees = np.rad2deg(phase_delay)
print(F"The time difference between the reference and the shifted signal is: {delay1} seconds."
      F" The phase difference is: {phase_delay_degrees}")
